...
class UserEntity extends BaseEntity
{
    ...
    /**
     * @ORM\OneToMany(targetEntity="ClientEntity", mappedBy="encoder", cascade={"remove"})
     */
    protected $encoderClients;
    ...
    public function __construct($data = null)
    {
        ...
        $this->encoderClients = new ArrayCollection();
    }
     ...
     */
    public function getEncoderClients(): Collection
    {
        return $this->encoderClients;
    }

    public function addEncoderClient(ClientEntity $encoderClient): self
    {
        if (!$this->encoderClients->contains($encoderClient)) {
            $this->encoderClients[] = $encoderClient;
            $encoderClient->setEncoder($this);
        }

        return $this;
    }

    public function removeEncoderClient(ClientEntity $encoderClient): self
    {
        if ($this->encoderClients->removeElement($encoderClient)) {
            // set the owning side to null (unless already changed)
            if ($encoderClient->getEncoder() === $this) {
                $encoderClient->setEncoder(null);
            }
        }
